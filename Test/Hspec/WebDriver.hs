...
import Test.Hspec.Core.Spec (Result(..), Item(..), Example(..), SpecTree, Tree(..), fromSpecList, runSpecM, ResultStatus(..))
...
instance Eq multi => Example (WdExample multi) where
    ...
    evaluateExample (WdPending msg) _ _ _ = return $ Result "Pending" (Pending Nothing msg)
    evaluateExample (WdExample multi (WdOptions {skipRemainingTestsAfterFailure}) wd) _ act _ = do
        ...
        return $ case (merr, mabort) of
            (True, _) -> Result "Pending" (Pending Nothing (Just "Previous example had an error"))
            (_, True) -> Result "Pending" $ Pending Nothing (Just "Session has been aborted")
            _ -> Result "Success" Success
